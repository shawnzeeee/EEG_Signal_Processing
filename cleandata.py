...
# Function to zero out closely packed classifications
def clean_flickering_classifications(dataframe, threshold=125):
    class_indices = dataframe[dataframe["Class"] != 0].index
    for i in range(len(class_indices) - 1):
        if class_indices[i + 1] - class_indices[i] <= threshold:
            dataframe.loc[class_indices[i], "Class"] = 0
            dataframe.loc[class_indices[i + 1], "Class"] = 0
    return dataframe

# Apply the cleaning function to the combined DataFrame
combined_df = clean_flickering_classifications(combined_df)
