...
//-------------------------------------------------------------------------------------
#define DATA_FILE "trial5.bin"
#define SAMPLE_RATE 250 // [Hz]
#define DURATION_DAQ 10 // [s]
#define DATA_READY_TRESHOLD_MS 30 // [ms]

// Definition of network specific stuff.
//-------------------------------------------------------------------------------------
#define HOST_IP "207.23.191.90"//"192.168.1.66" // Default address is the loopback address, else the ip of the computer running GDS.
#define HOST_PORT 50223     // The default port of GDS is 50223.
#define LOCAL_IP "209.87.56.60"// Default address is the loppback address, else the ip of the client machine.
...
void send_eeg_data(const float* data_buffer, size_t num_floats) {
	struct sp_port* port;
	const char* port_name = "/dev/ttyUSB0"; // Or "COM3" on Windows

	// Open port
	if (sp_get_port_by_name(port_name, &port) != SP_OK) {
		std::cerr << "Cannot find port " << port_name << std::endl;
		return;
	}
	...
	// Set settings: 1 MHz baud, 8N1, no flow control
	...
	// Write float EEG data byte-by-byte
	const uint8_t* byte_ptr = reinterpret_cast<const uint8_t*>(data_buffer);
	size_t total_bytes = num_floats * sizeof(float);

	int bytes_written = sp_blocking_write(port, byte_ptr, total_bytes, 1000);
	std::cout << "Wrote " << bytes_written << " bytes over serial." << std::endl;
