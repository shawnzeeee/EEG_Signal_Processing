...
columns_to_read = ["Channel_0", "Channel_1", "Channel_3"]  
...
n_components = min(3, numerical_data.shape[1]) 
...
for start in range(0, numerical_data.shape[0] - window_size, step_size):
    ...
    for i in range(n_components):
        corr, _ = pearsonr(blink_components[:window_components.shape[0], 0], window_components[:, i])
        if  abs(corr) > 0.1:
