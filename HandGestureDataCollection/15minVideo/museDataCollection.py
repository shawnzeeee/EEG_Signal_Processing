...
def play_balanced_videos_for(duration):
    ...
    while time.time() - start_time < duration:
        ...
        label = gesture_labels.get(os.path.basename(video_path), os.path.basename(video_path))
        class_index = video_list.index(video_path)
        csv_writer.writerow([timestamp, label, os.path.basename(video_path), class_index + 1])
        csv_file.flush()
