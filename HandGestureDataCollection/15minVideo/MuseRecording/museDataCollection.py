...
cycle_duration = 3 * 60    # 2 minutes per video session
break_duration = 0.5 * 60    # 2-minute break
total_duration = 10 * 60  # total session time

# --- CSV SETUP (moved up to be available for all functions) ---
filename = "test"
csv_path = os.path.join(script_dir, f"{filename}.csv")
# Open CSV at the start and write header
csv_file = open(csv_path, mode='w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(["Timestamp", "Video Label", "Filename", "Class Index"])
...
def play_video_then_countdown(path, gesture_index):
    ...
    for i in [3, 2, 1, "GO"]:
        ...
        if i == "GO":
            # Write to CSV at the exact GO cue
            timestamp = int(time.time() * 1000)  # 13-digit ms precision
            filename = os.path.basename(path)
            label = gesture_labels.get(filename, filename)
            class_index = video_list.index(path)
            csv_writer.writerow([timestamp, label, filename, class_index + 1])
            print("Writting classification: ", class_index + 1)
            csv_file.flush()
            send_gesture_classification(class_index + 1)
            # Show hold message for 6 seconds (no class 16 sent)
...
def play_balanced_videos_for(duration):
    ...
    while time.time() - start_time < duration:
        ...
        # Remove CSV writing from here (now handled at GO cue)
...
while time.time() - session_start < total_duration:
    ...
    csv_writer.writerow([break_timestamp, "Break", "BREAK", 15])
    csv_file.flush()
...
with open(csv_path, mode='w', newline='') as file:
    ...
    for entry in play_order:
        if len(entry) == 3 and entry[0] == "HOLD":
            _, timestamp, class_index = entry
            writer.writerow([timestamp, "Hold Period", "HOLD", class_index])
        else:
...
# Close the CSV file at the very end
csv_file.close()
