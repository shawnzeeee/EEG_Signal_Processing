...
def main():
    ...
    buffer_size = 4 * 250 * 2  # 2000 samples (4 channels, 2 seconds at 250Hz)
    eeg_buffer = []  # Will hold lists of 4 values each
    ...
    try:
        ...
        while True:
            ...
            eeg_buffer.append(sample[:4])  # Only take the first 4 channels
            if len(eeg_buffer) > buffer_size:
                eeg_buffer.pop(0)  # Keep buffer at 2 seconds (500 samples)
            if len(eeg_buffer) == 500:
                # Flatten the buffer to 1D (2000 values)
                flat_buffer = [val for row in eeg_buffer for val in row]
                # Write to shared memory
                shm.seek(0)
                shm.write(struct.pack('f'*2000, *flat_buffer))
